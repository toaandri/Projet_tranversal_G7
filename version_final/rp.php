...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ajouter_enseignant'])) {
    ...
    if (empty($id_enseignant) || empty($nom_enseignant) || empty($mot_de_passe)) {
        $error = "Tous les champs sont obligatoires.";
    } elseif (strlen($id_enseignant) > 10) {
        $error = "L'identifiant enseignant est trop long (maximum 10 caractères).";
    } else {
        try {
            // Vérifier si l'ID existe déjà
            $sql = "SELECT COUNT(*) FROM enseignant WHERE id_enseignant = :id_enseignant";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':id_enseignant', $id_enseignant);
            $stmt->execute();
            if ($stmt->fetchColumn() > 0) {
                $error = "L'identifiant enseignant existe déjà.";
            } else {
                // Insertion dans enseignant
                $sql = "INSERT INTO enseignant (id_enseignant, nom_enseignant, mot_de_passe) VALUES (:id_enseignant, :nom_enseignant, :mot_de_passe)";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(':id_enseignant', $id_enseignant);
